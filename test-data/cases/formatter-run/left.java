...
public final class Shipping {
    ...
    public Quote quote(Address origin, Address destination, Parcel parcel, Carrier carrier) {
        return rates.of(origin).to(destination).forParcel(parcel).via(carrier).cheapest();
    }

    public Money surcharge(Parcel parcel) {
        if (parcel.isOversized()) {
            return Money.of(25);
        }
        if (parcel.isFragile()) {
            return Money.of(10);
    ...
    public List<Carrier> carriersFor(Address destination, Parcel parcel, boolean expedited) {
        return registry.lookup(destination.country(), parcel.weightClass(), expedited);
    }
}
